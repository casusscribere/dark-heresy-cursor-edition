...
export class BasicActionManager {
    ...
    async _applyDamage(event) {
        ...
        for(const field of [damage, penetration, fatigue]) {
            if(field !== undefined && field !== null && field !== "" && Number.isNaN(Number.parseInt(field, 10))) {
    ...
    async assignDamageTool() {
        ...
        const sourceActorData = sourceToken ? sourceToken.actor : game.user.character;
    ...
    async sendItemVocalizeChat(data) {
        const html = await renderTemplate('systems/dark-heresy-3rd-edition/templates/chat/item-vocalize-chat.hbs', data);
        let chatData = {
            user: game.user.id,
            content: html,
            rollMode: game.settings.get('core', 'rollMode'),
            type: CONST.CHAT_MESSAGE_TYPES?.IC ?? CONST.CHAT_MESSAGE_TYPES?.OTHER ?? 0,
        };
