...
export class DarkHeresyItemContainer extends Item {
    ...
    async convertNestedToItems() {
        ...
        for (const nestedData of this.getNested()) {
            nestedData._id = nestedData._id ?? nestedData.id ?? foundry.utils.randomID();
    ...
    async deleteNestedDocuments(ids = []) {
        ...
        const newContained = containedItems.filter((itemData) => !ids.includes(itemData._id ?? itemData.id));
    ...
    async updateNestedDocuments(data) {
        ...
        let newContained = contained.map((existing) => {
            ...
            let theUpdate = data.find((update) => (update._id ?? update.id) === existingId);
            if (theUpdate) {
    ...
    prepareEmbeddedDocuments() {
        ...
        containedItems.forEach((idata) => {
            idata._id = idata._id ?? idata.id ?? foundry.utils.randomID();
