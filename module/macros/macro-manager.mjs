...
export async function createItemMacro(data, slot) {
    ...
    if (!itemId) return ui.notifications.warn(`Cannot create item macro without a valid item id`);

    let macroName = `${data.actorName}: ${payload.name}`;
    // Create the macro command
    const command = `game.dh.rollItemMacro("${data.actorId}", "${itemId}");`;
...
export function rollItemMacro(actorId, itemId) {
    ...
    const item = actor ? actor.items.get(itemId) ?? actor.items.find((i) => i._id === itemId) : null;
...
export async function createSkillMacro(data, slot) {
    ...
    const payload = getPayloadData(data);
    const { skill, speciality, name } = payload;
    ...
    const macro = await Macro.create({
        name: macroName,
        img: 'systems/dark-heresy-3rd-edition/icons/talents/red/r_36.png',
        type: 'script',
        command: command,
...
export async function createCharacteristicMacro(data, slot) {
    ...
    const payload = getPayloadData(data);
    const { characteristic, name } = payload;
    ...
    const macro = await Macro.create({
        name: macroName,
        img: 'systems/dark-heresy-3rd-edition/icons/talents/violet/p_05.png',
        type: 'script',
        command: command,
