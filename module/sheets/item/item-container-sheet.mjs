    ...
    async _onDrop(event) {
        ...
        try {
            ...
            if (data.type !== 'Item') {
                ...
                if (item?.documentName === "Item" && item.item) item = item.item;
                if (!item) {
                    game.dh.warn('Unable to resolve dropped item from UUID', data);
                    return false;
                }
                ...
                if (this.item.items.find((i) => i.id === item.id || i._id === item._id)) {
        ...
        if (item) {
            ...
            let canAdd = this.item.id !== item.id;
            ...
            while (parent && count < 10) {
                ...
                count += 1;
                canAdd = canAdd && parent.id !== item.id;
            ...
            if (this.canAdd(item)) {
                ...
                if (actor && (actor.type === 'acolyte' || actor.isToken)) await actor.deleteEmbeddedDocuments('Item', [item.id]);
                return false;
            }
            // Item is not accepted by this container -- place back onto actor
                ...
                if (actor && actor.type === 'acolyte') await actor.deleteEmbeddedDocuments('Item', [item.id]);
